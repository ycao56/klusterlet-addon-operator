...
{{- define "monitoring.prometheusScrapeTargets" }}
    ...
    - job_name: 'kubernetes-service-endpoints-with-tls'
      ...
      tls_config:
        ca_file: /etc/prometheus/secrets/{{ .Values.tls.ca.secretName }}/{{ .Values.tls.ca.certFieldName }}
      {{- if .Values.tls.client.existingSecretName }}
        cert_file: /etc/prometheus/secrets/{{ .Values.tls.client.existingSecretName }}/{{ .Values.tls.client.certFieldName }}
        key_file: /etc/prometheus/secrets/{{ .Values.tls.client.existingSecretName }}/{{ .Values.tls.client.keyFieldName }}
      {{- else }}
        cert_file: /etc/prometheus/secrets/{{ template "monitoring.fullname" . }}-client-certs/{{ .Values.tls.client.certFieldName }}
        key_file: /etc/prometheus/secrets/{{ template "monitoring.fullname" . }}-client-certs/{{ .Values.tls.client.keyFieldName }}
      {{- end }}
        insecure_skip_verify: true

  {{- if and .Values.nodeExporter.enabled .Values.tls.enabled }}
  ...
  {{- if .Values.prometheus.etcdTarget.enabled }}
    - job_name: etcd
      ...
      {{- if eq .Values.mode "managed" }}
        ca_file: /etc/prometheus/secrets/etcd-secret/etcd-ca
        cert_file: /etc/prometheus/secrets/etcd-secret/etcd-cert
        key_file: /etc/prometheus/secrets/etcd-secret/etcd-key
        insecure_skip_verify: true
      {{- end }}
